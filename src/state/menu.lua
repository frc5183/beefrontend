...
local container
local function load()
  local list_builder = gui.List(0, 40, 20, 660, 1260, gui.Color(0, 1, 0, 1), 0)
  local _menu = {
    name = "menu",
    login=list_builder.TextButton(640, 50, gui.Color(0, 1, 0, 1), "Login", 18, "center"),
    settings=list_builder.TextButton(640, 50, gui.Color(0, 1, 0, 1), "Settings", 18, "center"),
    exit=list_builder.TextButton(640, 50, gui.Color(0, 1, 0, 1), "Exit", 18, "center")
  }
  ...
  menu.settings:onClick(function(pt, button) if (menu.settings:contains(pt) and button == 1) then wait(0.05,
        function()settings.load() settings.setMenu(menu) state.switch(settings)  end) end end)
